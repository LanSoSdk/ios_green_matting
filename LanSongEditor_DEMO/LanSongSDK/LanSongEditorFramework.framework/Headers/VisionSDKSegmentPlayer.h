...
@interface VisionSDKSegmentPlayer : LSOObject


-(id)initWithURL:(NSURL *)videoUrl cutStartTime:(CGFloat)startTime cutEndTime:(CGFloat)endTime;



/// 设置或获取抠图系数, 在prepareAsync 前设置; 仅可设置一次;
@property (nonatomic, assign) int segmentLevel;
...
///当前ViewController 开始出现当前界面
- (void)viewDidAppear;

/// 当前ViewController不再显示当前界面;
 ...
 */
@property(nonatomic, copy) void(^segmentProgressBlock)(CGFloat ptsS);
 ...
 */
@property(nonatomic, copy) void(^exportProgressBlock)(CGFloat percent);
